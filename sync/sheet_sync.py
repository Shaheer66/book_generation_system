...
# SCOPES = ['https://www.googleapis.com/auth/spreadsheets']
# SERVICE_ACCOUNT_FILE = 'service_account.json' 
# SPREADSHEET_ID = os.getenv("SPREADSHEET_ID", '1fXm7IOnulEJ_NShPbo-kHRcP1ZfrX89-rudsYA144Vw')
    ...
    try:
        ...
        for ch in db_chapters:
            ...
            values.append([
                ...
                ch.get('title', ''), 
                content[:5000], # GSheets cell limit is ~50k, 5k is safe for preview
                ch.get('status', '')
            ])
...
def sync_new_books_to_db():
    """
    Bi-directional sync: 
    1. Pulls new books from Google Sheets and writes back the Supabase UUID.
    2. Listens for human status overrides (like 'approved') and pushes them to the DB.
    """
    service = get_sheets_service()
    sheet = service.spreadsheets()
    supabase = get_supabase_client()
    
    # Read A to G to capture all columns safely
    result = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range="Books_Overview!A2:G").execute()
    rows = result.get('values', [])
    
    # Fetch current DB state to prevent overwriting human actions
    db_books = supabase.table("books").select("id, status").execute().data
    db_book_map = {book['id']: book['status'] for book in db_books if book.get('id')}
    
    for i, row in enumerate(rows):
        if not row: continue # Skip completely empty rows
        
        # Safely extract data, handling shortened arrays if cells are empty
        book_id = row[0].strip() if len(row) > 0 else ""
        title = row[1] if len(row) > 1 else "Untitled"
        notes = row[2] if len(row) > 2 else ""
        sheet_outline = row[3] if len(row) > 3 else ""
        sheet_status = row[4].strip().lower() if len(row) > 4 else ""
        editor_email = row[6] if len(row) > 6 else os.getenv("ADMIN_EMAIL", "admin@example.com")
        
        row_number = i + 2 
        
        # CASE 1: New Book (Column A is empty)
        if not book_id:
            try:
                data = supabase.table("books").insert({
                    "title": title, 
                ...
                # Immediately write new_id back to Column A
                sheet.values().update(
                    spreadsheetId=SPREADSHEET_ID,
                    range=f"Books_Overview!A{row_number}",
                    valueInputOption="USER_ENTERED",
        ...
        # CASE 2: Existing Book - Check for Human Interventions (The Race Condition Fix)
        else:
            db_status = db_book_map.get(book_id)
            
            # If a human typed 'approved' or 'rejected' in the sheet, but the DB doesn't know yet
            if sheet_status in ['approved', 'rejected'] and db_status != sheet_status:
                try:
                    # Push the human's status AND their potential outline edits to the DB
                    supabase.table("books").update({
                        "status": sheet_status,
                        "outline_content": sheet_outline
                    }).eq("id", book_id).execute()
                    
                    logger.info(f"Human Override: Book {book_id} updated to '{sheet_status}' via Google Sheets.")
                except Exception as e:
                    logger.error(f"Failed to push human override for book '{book_id}': {e}")

def sync_db_to_sheets():
    """Pushes AI-generated outlines and system statuses from Supabase back to Google Sheets."""
    try:
        supabase = get_supabase_client()
        service = get_sheets_service()
        sheet = service.spreadsheets()
        ...
        for i, row in enumerate(rows):
            ...
            if book_id in db_book_map:
                ...
                # Only push DB data to the sheet if it's NOT a human action waiting to be processed
                sheet_status = row[4].strip().lower() if len(row) > 4 else ""
                if sheet_status in ['approved', 'rejected']:
                    continue # Skip overwriting this row, let sync_new_books_to_db handle it
        ...
        if batch_data:
            ...
            # Suppressed the noisy log here unless debugging to keep your terminal clean, but left it for POC
            logger.info("Successfully pushed database updates back to Google Sheets.")
